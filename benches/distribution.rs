...
fn benchmark_distribution(c: &mut Criterion) {
    ...
    for price in &btc_prices {
        group.bench_with_input(
            ...
            price,
            |b, &price| {
                b.iter(|| generate_distribution(black_box(price), black_box(0)));
            },
        );
    }
    ...
    for cap in &caps {
        group.bench_with_input(
            ...
            cap,
            |b, &cap| {
                b.iter(|| generate_distribution(black_box(base_price), black_box(cap)));
            },
        );
    }
...
fn benchmark_edge_cases(c: &mut Criterion) {
    ...
    group.bench_function("minimum_viable_cap", |b| {
        b.iter(|| generate_distribution(black_box(price), black_box(min_cap)));
    });

    // Very high BTC price
    group.bench_function("high_btc_price", |b| {
        b.iter(|| generate_distribution(black_box(1_000_000_00), black_box(0)));
    });

    // Very low BTC price
    group.bench_function("low_btc_price", |b| {
        b.iter(|| generate_distribution(black_box(1_00), black_box(0)));
    });
